...
object FaceIdentifier {
  ...
  def find(image: Mat): Rect = {
    val cascadeClassifier: CascadeClassifier = new CascadeClassifier(getClass.getClassLoader.getResource("haarcascade/haarcascade_frontalface_alt2.xml").getPath)
